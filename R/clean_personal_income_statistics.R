...
clean_LGA_personal_income_statistics <- function(file_path) {
  ...
  filled_headers <- zoo::na.locf(cleaned_headers, na.rm = FALSE)
  LGA_data[1, ] <- as.list(filled_headers)

  combined_headers <- mapply(paste,
                             LGA_data[1, ],
                             LGA_data[2, ],
                             MoreArgs = list(sep = ":"),
  ...
  pivoted_LGA_data <- cleaned_LGA_data |>
    tidyr::pivot_longer(cols_to_pivot, names_to = "Measure", values_to = "Values") |>
    dplyr::mutate(
      Values = dplyr::replace_values(Values, "np" ~ NA_character_),
      Values = as.numeric(Values)
    ) |>
    tidyr::separate_wider_delim(Measure, ":", names = c("Measure", "Year")) |>
    dplyr::filter(Measure %in% c("Earners", "Median age of earners", "Median")) |>
    dplyr::mutate(
