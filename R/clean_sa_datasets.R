...
clean_pre_2020_Q3_sa_sheet <- function(raw_sheet, file_name) {
  ...
  required_columns <- raw_data |>
    ...
    tidyr::fill(`Statistical Local Area (2011)`, .direction = "down") |> # This gives the very last row which contains all of SA stastics a Total so we can keep it for.
    dplyr::filter(`Statistical Local Area (2011)` == "Total") |>
    dplyr::select(-`Statistical Local Area (2011)`) |>
